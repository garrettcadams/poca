...
let setCookie2 = (cname, cvalue, exdays) => {
  var d = new Date();
...
class DisplayMode {
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data) {
              ...
              setCookie2('display_mode', 'list', 0.1);
            }
          }
        });
        return false;
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data) {
              ...
              setCookie2('display_mode', 'grid', 0.1);
            }
          }
        });
        return false;
  ...
  _getDisplayMode() {
    ...
    if (Utils.getCookie('display_mode') != undefined && Utils.getCookie('display_mode') !== "") {
      if (Utils.getCookie('display_mode') == 'grid') {
        let mode = $('.display-mode').find("button.grid");
        mode.parent().children().removeClass('active');
        mode.addClass('active');
        $('.tbay-filter').parents('#content').find('.products').addClass('products-' + Utils.getCookie('display_mode'));
      }

      if (Utils.getCookie('display_mode') == 'list') {
