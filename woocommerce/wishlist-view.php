...
<form id="yith-wcwl-form" action="<?php echo esc_url($form_action); ?>" method="post" class="woocommerce">
    ...
    ?>
        <?php 
            $class_title = ( $is_custom_list ) ? 'wishlist-title-with-form' : '';
        ?>
        <div class="wishlist-title <?php echo esc_attr( $class_title ); ?>">
            ...
            <?php if( $is_custom_list ): ?>
                <a class="btn button show-title-form">
                    ...
                    <?php esc_html_e( 'Edit title', 'puca' ) ?>
                </a>
            <?php endif; ?>
        </div>
        <?php if( $is_custom_list ): ?>
            <div class="hidden-title-form">
                ...
                <button>
                    <?php echo apply_filters( 'yith_wcwl_save_wishlist_title_icon', '<i class="icon-heart"></i>' )?>
                    <?php esc_html_e( 'Save', 'puca' )?>
                </button>
                <a class="hide-title-form btn button">
                    <?php echo apply_filters( 'yith_wcwl_cancel_wishlist_title_icon', '<i class="fa fa-remove"></i>' )?>
                    <?php esc_html_e( 'Cancel', 'puca' )?>
                </a>
            </div>
        <?php endif; ?>
    <?php
	...
	<table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr( $wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
        ...
        <tr>
            ...
            <th class="product-name">
                <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'puca' ) ) ?></span>
            </th>
            ...
            <?php if( $show_price ) : ?>
                ...
                <th class="product-price">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'puca' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_stock_status ) : ?>
                ...
                <th class="product-stock-status">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'puca' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_last_column ) : ?>
                ...
                <th class="product-add-to-cart"><?php esc_html_e( 'Add to cart', 'puca' ); ?></th>
            ...
            <?php if( $is_user_owner ): ?>
                <th class="product-remove"><?php esc_html_e( 'Remove', 'puca' ); ?></th>
            <?php
                $column_count ++;
            endif;
            ?>
        ...
        if( count( $wishlist_items ) > 0 ) :
                    ...
                    <tr id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']); ?>" data-row-id="<?php echo esc_attr($item['prod_id']); ?>">
                        ...
                        <td class="product-thumbnail">
                            <a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
                                ...
                                $attachment_ids = get_post_thumbnail_id( $item['prod_id'] );
                                $attachment = wp_get_attachment_image_src($attachment_ids, 'woocommerce_thumbnail' ); ?>    
                                <img src="<?php echo esc_url($attachment[0]); ?>" class="wishlist-image"  />
                            </a>
                        </td>
                        ...
                        <?php if( $show_price ) : ?>
                            <td class="product-price">
                                <?php 
                                    $product_price = ($product->get_price()) ? $product->get_price_html() : apply_filters( 'yith_free_text', esc_html__( 'Free!', 'puca' ) ); 
                        ...
                        <?php if( $show_stock_status ) : ?>
                            <td class="product-stock-status">
                                ...
                                <?php 
                                    $stock_html = ($stock_status == 'out-of-stock') ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'puca' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'puca' ) . '</span>';
                                ?>
                                <?php echo trim($stock_html); ?>
                            </td>
                        <?php endif ?>
                        ...
                        <td class="product-add-to-cart">
	                        ...
	                        if( $show_dateadded && isset( $item['dateadded'] ) ):
								echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'puca' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
	                        endif;
	                        ?>
	                        ...
	                        <select class="change-wishlist selectBox">
		                        <option value=""><?php esc_html_e( 'Move', 'puca' ) ?></option>
		                        <?php
		                        ...
		                        ?>
			                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
				                        ...
				                        if( $wl['wishlist_privacy'] == 1 ){
					                        $wl_privacy = esc_html__( 'Shared', 'puca' );
				                        }
				                        elseif( $wl['wishlist_privacy'] == 2 ){
					                        $wl_privacy = esc_html__( 'Private', 'puca' );
				                        }
				                        else{
					                        $wl_privacy = esc_html__( 'Public', 'puca' );
				                        }
	                        ...
	                        <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_html_e( 'Remove this product', 'puca' ) ?>"><?php esc_html_e( 'Remove', 'puca' ) ?></a>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="product-remove">
                            <div>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php esc_html_e( 'Remove this product', 'puca' ) ?>">&#215;</a>
                            </div>
                        </td>
                        <?php endif; ?>
            ...
            <p class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products were added to the wishlist', 'puca' ) ) ?></p>
        <?php
        endif;
	        ...
	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
	            <?php if( $show_cb ) : ?>
		            <div class="custom-add-to-cart-button-cotaniner">
		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_token ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', esc_html__( 'Add the selected products to the cart', 'puca' ) ) ?></a>
		            </div>
	            <?php endif; ?>
	            ...
	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
		            <div class="ask-an-estimate-button-container">
                        <?php 
                            $ask_url = ( $additional_info || ! is_user_logged_in() ) ? '#ask_an_estimate_popup' : $ask_estimate_url;
                            $ask_red = ( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : ''
                        ?>
	                    <a href="<?php echo esc_attr($ask_url); ?>" class="btn button ask-an-estimate-button" <?php echo esc_attr( $ask_red ); ?> >
	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_icon', '<i class="fa fa-shopping-cart"></i>' )?>
	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'puca' ) ) ?>
	                </a>
		            </div>
	            <?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_url($ask_estimate_url) ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! is_user_logged_in() ): ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', esc_html__( 'Your email', 'puca' ) ) ?></label>
