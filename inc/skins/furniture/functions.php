...
if ( !function_exists('puca_tbay_private_size_image_setup') ) {
	function puca_tbay_private_size_image_setup() {
		...
		*/
		...
		update_option('puca_avatar_post_carousel', 70, 70, true);
		add_image_size('puca_avatar_post_carousel', 70, 70, true); //(cropped)

		// Post Thumbnails Size
		set_post_thumbnail_size(370, 240, true); // Unlimited height, soft crop
...
if ( !function_exists('puca_tbay_private_menu_setup') ) {
	function puca_tbay_private_menu_setup() {
		...
		register_nav_menus( array(
			...
			'nav-category-menu'  => esc_html__( 'Nav Category Menu', 'puca' ),
...
if ( !function_exists('puca_fonts_url') ) {
	function puca_fonts_url() {
	    ...
	    */
	    ...
	    $Nunito 		= _x( 'on', 'Nunito font: on or off', 'puca' );
	 
	    if ( 'off' !== $Nunito || 'off' !== $Lato ) {
	        $font_families = array(); 
	        ...
	        if ( 'off' !== $Nunito ) {
	            $font_families[] = 'Nunito:200,400,600,700,800,900';
	        }
...
if ( !function_exists('puca_tbay_private_enqueue_styles') ) {
    function puca_tbay_private_enqueue_styles() {
    	...
    	wp_register_script( 'jquery-countdowntimer', PUCA_SCRIPTS . '/timecircles' . $suffix . '.js', '1.0', true );
	...
	function puca_tbay_widgets_init() {
		...
		if( defined('PUCA_REDUX_FRAMEWORK_ACTIVED') && PUCA_REDUX_FRAMEWORK_ACTIVED ) {
			register_sidebar( array(
				'name'          => esc_html__( 'Top Info', 'puca' ),
				'id'            => 'top-info',
				'description'   => esc_html__( 'Add widgets here to appear in top info.', 'puca' ),
			...
			register_sidebar( array(
				'name'          => esc_html__( 'Top Social', 'puca' ),
				'id'            => 'top-social',
				'description'   => esc_html__( 'Add widgets here to appear in top social', 'puca' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h2 class="widget-title">',
				'after_title'   => '</h2>',
			...
			register_sidebar( array(
				'name'          => esc_html__( 'Top Copyright', 'puca' ),
				'id'            => 'top-copyright',
				'description'   => esc_html__( 'Add widgets here to appear in top copyright.', 'puca' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h2 class="widget-title">',
				'after_title'   => '</h2>',
	        	...
	        	switch ( $blog_archive_layout ) {
							case 'main-v1':
						 	...
						 	case 'main-v3':
						 		...
						 		break;							 	
						 	case 'main-v4':
						 		$configs['main'] 			= array( 'class' => 'archive-full v4' );
						 		$configs['container_full'] 	= true;
						 		$configs['columns'] 		= 4;
						 		$configs['image_sizes'] 	= 'medium';
						 		break;							 	
						 	case 'left-main-v1':
