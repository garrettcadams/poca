...
if ( !function_exists('puca_tbay_before_woocommerce_init') ) {
    function puca_tbay_before_woocommerce_init() {
        if( isset($_GET['display']) && ($_GET['display']=='list' || $_GET['display']=='grid') ){  
            setcookie( 'puca_woo_mode', trim($_GET['display']) , time()+3600*24*100,'/' );
            $_COOKIE['puca_woo_mode'] = trim($_GET['display']);
        }
    }
...
//Count all product
if ( ! function_exists( 'puca_total_product_count' ) ) {
    function puca_total_product_count() {
...
if ( !function_exists('puca_tbay_woocommerce_redirect_page_cart_mobile') ) {
    // define the woocommerce_add_to_cart_redirect callback 
    function puca_tbay_woocommerce_redirect_page_cart_mobile( $wc_get_cart_url ) { 
...
if ( !function_exists('puca_tbay_woocommerce_disable_redirect_page_cart_mobile') ) {
    // define the woocommerce_add_to_cart_redirect callback 
    function puca_tbay_woocommerce_disable_redirect_page_cart_mobile( ) { 

        $active = puca_tbay_get_config('redirect_add_to_cart', false);

        if( $active && wp_is_mobile() ) {
            add_filter( 'woocommerce_add_to_cart_redirect', 'puca_tbay_woocommerce_redirect_page_cart_mobile', 10, 1 );   
        }
        
    }; 
}
add_action( 'wp_loaded', 'puca_tbay_woocommerce_disable_redirect_page_cart_mobile', 10, 1 ); 
